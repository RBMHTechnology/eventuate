...
The user guide only scratches the surface of Eventuate. You can find further details in the :ref:`reference` and the `API docs`_.
...
.. includecode:: code/QuickTourDoc.scala
...
.. note::
   Only concurrent updates to replicas with the same ``aggregateId`` may conflict. Concurrent updates to actors with different ``aggregateId`` do not conflict (unless an application overrides the default event routing rules). 
...
.. note::
   Interactive conflict resolution requires agreement between replicas affected by a given conflict. Only one of them may emit the ``Resolved`` event. This does not necessarily mean distributed leader (= resolver) election or lock acquisition but can also rely on static rules such as *only the initial creator location of an aggregate is allowed to resolve the conflict*. This rule is implemented in the :ref:`example-application`.
...
.. [#] ``EventsourcedActor``\ s and ``EventsourcedView``\ s that have no ``aggregateId`` defined can consume events from all other actors on the same event log. 
