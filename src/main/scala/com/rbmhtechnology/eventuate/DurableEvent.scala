 ...
 * @param systemTimestamp Wall-clock timestamp, generated by the log that initially persisted the emitted event. The id of
 *                        that log is `persistLogId`.
 * @param vectorTimestamp Vector timestamp, generated by the log that initially persisted the emitted event. The id of
 *                        that log is `persistLogId`.
 * @param persistLogId Id of the log to which the emitted event has been initially written.
 * @param sourceLogId Id of the source log from last replication. Equal to `targetLogId` if not replicated yet.
...
case class DurableEvent(
  ...
  persistLogId: String = UndefinedLogId,
