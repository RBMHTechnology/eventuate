...
trait MultiLocationSpec extends LocationCleanup with BeforeAndAfterEach {
  ...
  def location(name: String, customPort: Int = 0, customConfig: Config = ConfigFactory.empty()): Location = {
    val location = new Location(locationId(name), logFactory, customPort, customConfig)
...
class Location(val id: String, logFactory: String => Props, customPort: Int, customConfig: Config) {
  ...
  val system: ActorSystem =
    ActorSystem(ReplicationConnection.DefaultRemoteSystemName, LocationConfig.create(customPort, customConfig))
