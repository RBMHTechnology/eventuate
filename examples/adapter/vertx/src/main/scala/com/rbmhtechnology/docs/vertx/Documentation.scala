...
object Documentation {
  ...
  val sourceLog: ActorRef      = endpoint.logs("sourceLog")
  val destinationLog: ActorRef = endpoint.logs("destinationLog")

  val config: VertxAdapterConfig = VertxAdapterConfig()
    .addProducer(
  ...
  val adapter: VertxAdapter = VertxAdapter(config, vertx, storageProvider)(actorSystem)
